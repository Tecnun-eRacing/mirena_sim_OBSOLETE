...
void MirenaCam::set_near_clip(float p_near) {
    near_clip = p_near;
    if (camera) camera->set_near(near_clip);
}
...
void MirenaCam::set_far_clip(float p_far) {
    far_clip = p_far;
    if (camera) camera->set_far(far_clip);
}
...
void MirenaCam::set_projection_type(Camera3D::ProjectionType p_type) {
    ...
    if (camera) camera->set_projection(projection_type);
}
...
void MirenaCam::set_size_ortho(float p_size) {
    size_ortho = p_size;
    if (camera) camera->set_size(size_ortho);
}
...
void MirenaCam::set_h_offset(float p_offset) {
    h_offset = p_offset;
    if (camera) camera->set_h_offset(h_offset);
}
...
void MirenaCam::set_v_offset(float p_offset) {
    v_offset = p_offset;
    if (camera) camera->set_v_offset(v_offset);
}
...
void MirenaCam::set_doppler_tracking(bool p_enable) {
    ...
    if (camera) {
        camera->set_doppler_tracking(doppler_tracking ? 
            Camera3D::DOPPLER_TRACKING_IDLE_STEP : 
            Camera3D::DOPPLER_TRACKING_DISABLED);
    }
}
